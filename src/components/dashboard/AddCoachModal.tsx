...
export default function AddCoachModal({
  ...
  if (created) {
    return (
      <Modal title="Compte coach créé" onClose={() => onCreated()}>
        <div className="flex flex-col gap-4">
          <p className="text-sm text-gray-600">
            Le compte coach de <strong>{created.name}</strong> est prêt, avec le même accès complet que le tien.
            Communique-lui ces identifiants (aucun email n'a été envoyé) :
          </p>
  ...
  return (
    <Modal title="Ajouter un coach" onClose={onClose}>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <p className="text-xs text-gray-400 -mt-1">
          Ce compte aura un accès complet à l'application, identique au tien (tous les athlètes, la programmation, la
          bibliothèque, les groupes).
        </p>
        ...
        >
          {saving ? "Création..." : "Créer le compte coach"}
        </button>
      </form>
    </Modal>
  );
